  ...
  return (
    <section className="bg-white py-[72px] sm:py-[100px]" id="company">
      <div className="mx-auto max-w-[1440px] px-[16px] sm:px-[40px] lg:px-[56px]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-[10px] sm:gap-[20px]">
          {items.map((item, i) => (
            <motion.div key={item.stat} initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ delay: i * 0.1, duration: 0.65, ease }}
              className="rounded-[14px] sm:rounded-[24px] bg-[#f5f4f0] p-[16px] sm:p-[40px] flex flex-col">
              <div className="text-[18px] sm:text-[28px] mb-[10px] sm:mb-[24px]">{item.icon}</div>
              <div className="text-[28px] sm:text-[clamp(40px,6vw,72px)] font-[750] tracking-[-0.05em] text-[#0a0a0a] leading-[1.0] mb-[8px] sm:mb-[20px]">{item.stat}</div>
              <h3 className="text-[12px] sm:text-[18px] font-[700] tracking-tight text-[#0a0a0a] mb-[6px] sm:mb-[12px] leading-[1.2]">{item.title}</h3>
              <p className="hidden sm:block text-[14px] leading-[1.68] text-black/48 flex-1 mb-[28px]">{item.desc}</p>
              <a href="#" className="inline-flex items-center gap-[4px] text-[11px] sm:text-[13px] font-[600] text-black/38 hover:text-black transition-colors">{item.link} →</a>
            </motion.div>
          ))}
        </div>
      </div>
