...
export default function CustomerCarousel() {
  ...
  return (
    <section className="bg-[#f5f4f0] py-[72px] sm:py-[100px] overflow-hidden" id="industries" style={{ WebkitOverflowScrolling: "touch" }}>
      {/* Heading */}
      ...
      {/* Row 1 — scroll left */}
      <div className="mb-[10px] sm:mb-[16px] flex gap-[10px] sm:gap-[16px]" style={{ animation: "marquee 48s linear infinite", width: "max-content", willChange: "transform" }}>
        {[...customers, ...customers].map((c, i) => (
          <div
            key={`r1-${i}`}
            className="shrink-0 w-[190px] sm:w-[320px] rounded-[14px] sm:rounded-[20px] bg-white border border-black/[0.07] p-[16px] sm:p-[28px]"
          >
            <div className="mb-[12px] sm:mb-[20px] h-[32px] sm:h-[40px] w-[56px] sm:w-[76px] rounded-[8px] sm:rounded-[10px] bg-black/[0.05] flex items-center justify-center text-[9px] sm:text-[11px] font-[700] tracking-[0.05em] text-black/35">
              {c.logo}
            </div>
            <p className="text-[11px] sm:text-[13px] font-[600] text-[#0a0a0a] mb-[5px] sm:mb-[8px] leading-[1.3]">{c.name}</p>
            <p className="text-[11px] sm:text-[13px] leading-[1.6] text-black/45">{c.desc}</p>
          </div>
        ))}
      </div>

      {/* Row 2 — scroll right */}
      <div className="flex gap-[10px] sm:gap-[16px]" style={{ animation: "marquee-r 48s linear infinite", width: "max-content", willChange: "transform" }}>
        {[...row2, ...row2].map((c, i) => (
          <div
            key={`r2-${i}`}
            className="shrink-0 w-[190px] sm:w-[320px] rounded-[14px] sm:rounded-[20px] bg-white border border-black/[0.07] p-[16px] sm:p-[28px]"
