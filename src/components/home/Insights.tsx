  ...
  return (
    <section className="bg-[#f5f4f0] py-[72px] sm:py-[100px]">
      <div className="mx-auto max-w-[1440px] px-[16px] sm:px-[40px] lg:px-[56px]">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-[10px] sm:gap-[16px]">
          {articles.map((article, i) => (
            <motion.div key={article.title} initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.6, ease }}
              className="group rounded-[12px] sm:rounded-[20px] bg-white overflow-hidden cursor-pointer hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition-shadow duration-300">
              <div className="h-[90px] sm:h-[200px] flex items-center justify-center" style={{ backgroundColor: article.bg }}>
                <span className="text-[32px] sm:text-[60px] opacity-25">{article.emoji}</span>
              </div>
              <div className="p-[12px] sm:p-[24px]">
                <span className="text-[9px] sm:text-[10px] font-[700] tracking-[0.09em] uppercase mb-[5px] sm:mb-[10px] block" style={{ color: "#22c55e" }}>{article.tag}</span>
                <h3 className="text-[11px] sm:text-[15px] font-[700] tracking-tight text-[#0a0a0a] leading-[1.35] mb-[6px] sm:mb-[12px] group-hover:text-black/65 transition-colors">{article.title}</h3>
                <span className="text-[10px] sm:text-[12px] text-black/32 font-[500]">{article.meta}</span>
              </div>
            </motion.div>
          ))}
        </div>
